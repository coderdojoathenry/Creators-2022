  ...
  public class AnimalController : MonoBehaviour
  {
    ...
    private void Update()
    {
      if (!this.gameObject.CompareTag("Chicken")) // If animal the script is attached to is not a chicken, animate 4 legs and body.
      {
        ...
        // Wander
        Vector3 newHorizPos = transform.position + ((transform.forward * Time.deltaTime) * movSpeed);
        Vector3 newPosRayStart = newHorizPos + (Vector3.up * 200);
        RaycastHit hitInfo = new RaycastHit();

        LayerMask lm = LayerMask.GetMask(GroundLayerName);
        if (Physics.Raycast(newPosRayStart, Vector3.down, out hitInfo, 300, lm))
        {
